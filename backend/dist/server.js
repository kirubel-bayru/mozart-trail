...
app.get('/api/music/:locationId', async (req, res) => {
    ...
    if (!base) {
        return res.status(501).json({
            ...
            message: 'Set MUSIC_API_URL on the backend to your music provider. Frontend falls back to sample tracks.',
            locationId: req.params.locationId,
        });
    }
    try {
        const upstream = await fetch(`${base}/locations/${encodeURIComponent(req.params.locationId)}`, { headers: { Accept: 'application/json' } });
        if (!upstream.ok) {
